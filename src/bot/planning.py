...
# Planning tool names that should interrupt the stream and require user interaction
...
class PlanningHandler:
    ...
    async def present_question(
        ...
        kb = _build_question_keyboard(user_id, q_idx, options, is_multi, chosen)
    ...
    async def handle_ask_callback(
        ...
        if choice == "done" and is_multi:
            ...
            await _maybe_resume_or_next(
                update, context, state, query.message, resume_fn,
            )
            return
        ...
        if is_multi:
            ...
            # Re-render buttons
            await query.answer()
            kb = _build_question_keyboard(
                query.from_user.id, q_idx, options, is_multi=True, chosen=chosen,
            )
            try:
    ...
    async def handle_text_for_planning(
        ...
        if state.type == "user_question" and state.awaiting_other_text:
            ...
            _advance_question(state)
            await _maybe_resume_or_next(
                update, context, state, update.message, resume_fn,
            )
            return True
...
def _build_question_keyboard(
    ...
    """Build the inline keyboard for a question's options."""
...
async def _maybe_resume_or_next(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    state: PlanningState,
    reply_target: Any,
    ...
    """If all questions answered, resume Claude; otherwise show next question."""
    questions = state.questions or []
    if state.current_question_idx >= len(questions):
        prompt = _format_answers_prompt(state)
        _clear_state(context)
        await reply_target.reply_text("Thanks! Resuming...")
