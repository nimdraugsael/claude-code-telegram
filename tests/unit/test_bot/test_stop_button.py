...
class TestStopButtonOnProgress:
    ...
    async def test_progress_message_has_stop_button(self, orchestrator, settings):
        """_process_text_with_claude sends progress_msg with Stop keyboard."""
        user_id = 42
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("src.bot.utils.formatting.ResponseFormatter") as MockFmt:
                    ...
                    await orchestrator._process_text_with_claude(
                        update, context, "test"
                    )
    ...
    async def test_active_request_cleaned_up_after_error(self, orchestrator, settings):
        ...
        with patch(
            ...
            with patch(
                ...
                await orchestrator._process_text_with_claude(update, context, "test")
...
class TestSDKInterrupt:
    """Test interrupt_event triggers client.interrupt() in execute_command."""

    async def test_interrupt_event_calls_client_interrupt(self, sdk_manager, tmp_path):
        """Setting the interrupt_event should call client.interrupt()."""
        ...
        # Messages arrive slowly so interrupt can fire
        client = _mock_client(assistant_msg, result_msg, delay=0.1)
        ...
        with patch("src.claude.sdk_integration.ClaudeSDKClient", return_value=client):
            # Fire interrupt shortly after start
